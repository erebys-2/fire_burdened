...
class player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, speed, hp, stamina, hits_tanked, stamina_usage_cap, ini_vol, camera_offset):
        ...
        animation_types = ('idle', 'run', 'jump', 'land', 'squat', 'hurt', 
                           ...
                           'atk1_5', 'jump_up', 'jump_down', 'flip', 'sprint')
    ...
    def move(self, pause_game, moveL, moveR, world_solids, world_rect, x_scroll_en, y_scroll_en, half_screen, screenH, the_sprite_group, ccsn_chance):
        ...
        #move
        if (self.action == 1 or self.action == 22) and self.frame_index%3 == 0:
            ...
            if self.shoot:
                ...
                if change_once:
                    ...
                    if self.in_air and self.hold_jump and self.consecutive_upstrike <= self.upstrike_limit:
                        self.atk1_alternate = True
                    elif self.in_air and (not self.hold_jump or self.consecutive_upstrike > self.upstrike_limit):
                        self.atk1_alternate = False
                    elif not self.in_air:
                        self.atk1_alternate = not self.atk1_alternate
                    change_once = False
    ...
    def update_action(self, new_action):
        ...
        if new_action != self.action:
            ...
            if (self.action == 7 or self.action == 8) and (self.rolling or self.action in (9, 21)):
                self.crit = False
                self.heavy = False
                self.atk1_stamina_cost = self.atk1_default_stam
                self.atk1 = False
            ...
            elif self.action in (9, 21) and (new_action == 7 or new_action == 8) and self.atk1:
                self.heavy = False
                if do_not_crit:
                    self.atk1 = False
                    self.atk1_kill_hitbox()
            ...
            elif self.action not in (9, 21) and (new_action == 7 or new_action == 8):
                self.crit = False
